...
class ImageConverter(MediaConverter):
    ...
    def _get_llm_description(self, local_path, extension, client, model, prompt=None):
        if prompt is None or prompt.strip() == "":            
            ...
            + "If the image contains a table, return the contents of the table in markdown table syntax too, do not use 3 backticks sections ``` unless it is needed" \
            + "For tables in markdown syntax, keep rows in the same line, do not add extra white space at end of text, and use the following format: " \
